...
- 生成・編集後は `pptx_get_preview_images` で全ページをClaudeへ渡し、問題時は宣言型仕様を最大3回まで修正する。画像確認では作成者の意図を弁護に使わず、独立した検収者として各ページを描画、視覚的重心、主張、タイトル対応、全体整合の5項目でPASS／FAIL判定する。通常の欠け・重なりに加え、切れたコネクタ、関係のない二論点の同居、意図のない左右の過密・空疎、見出しだけで話が追えるか、読む順序が一意か、1ブロック1論点か、強調色が概ね15%以内か、本文が9pt未満に見えないかを確認し、1項目でもFAILなら完成扱いにしない。
...
- 既定テンプレートの`cover-title`／`cover-subtitle`（54px／27px）と`body-title`／`body-claim`（50px／27px）のCSS契約は、finish後のNode rendererまで遅延させず、`pptx_add_visual_slides_to_draft`のbatch受理前に検証する。不正batchは1ページも受理しない。
- `visual_authored_html_invalid`と、旧jobでmodel-authored HTML/CSS由来と特定できる`visual_renderer_failed`は、失敗jobのpayloadをcheckpointとして`pptx_refine_visual_slide`で該当1ページだけ修正可能にする。この失敗後の暗黙的なDesign Brief再検証／全体startは拒否し、status tool結果へ正確なfailed job IDを使う復旧指示を付ける。
- PptxGenJSを含む外部レンダラーの生成物は、LibreOfficeで表示できてもPowerPoint互換とは限らない。生成直後と企業テンプレートへの合成後にOpenXmlValidatorを通し、新規検証エラーがある成果物は配布しない。
- `dom-to-pptx`のNode exporterはsandboxなしChromiumを起動するため、検証・再構築済みのoffline HTMLだけを入力し、非root、read-only、全capability削除、internal networkを維持する。runtimeでbrowserをdownloadせず、イメージへ固定インストールしたChromiumだけを使う。Chromium起動時のWS endpoint待機タイムアウトだけは同一renderer入力で1回リトライし、HTML/CSS検証失敗や依存不足などの恒久エラーはリトライしない。
