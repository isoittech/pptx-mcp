...
public sealed class PowerPointTools
{
    ...
    [McpServerTool(Name = "pptx_get_preview_images", ReadOnly = true, Idempotent = true),
     Description("成功済みジョブのスライド画像をClaude自身の視覚確認用に返します。全ページ確認はスライド番号順の連続4枚ずつ、最後の端数だけ1〜3枚で取得し、同じjobの同じページを重複取得しません。作成者から独立した厳しい検収者として、各ページを描画の完全性、構図と視覚的重心、主張の一貫性、タイトルと本文の対応、資料全体との整合の5項目でPASS／FAIL判定してください。文字切れがないだけでPASSにせず、切れた矢印や四角い断片、関係のない二論点の同居、意図のない左右の過密・空疎はFAILです。厳密なプレースホルダー資料はpptx_refine_deck、白紙またはブランドVisual Deckはpptx_refine_visual_slideへ問題ページを1枚ずつ渡します。最大3巡で収束させ、このツールを呼ばずに視覚確認済みと述べてはいけません.")]
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                Text = "Enter independent acceptance-review mode for the returned slides. Do not defend a result with authoring intent or its HTML/CSS. For every slide, make an internal PASS or FAIL decision supported by observed evidence for five gates: render integrity; composition and visual center of gravity; claim coherence; title-to-body agreement; and deck-wide consistency. Absence of clipping alone is never sufficient for PASS. Treat broken, interrupted, or square-fragment connectors; ambiguous direction; unrelated topics placed side by side without a claim, causal, comparison, or containment relationship; and an unintentionally crowded or empty side as hard failures. Asymmetry is allowed only when it clearly supports hierarchy or reading order; equal outer box dimensions alone do not prove balance. Also evaluate overflow, overlap, legibility, spacing, alignment, contrast, hierarchy, density, visual variety, heading-story continuity, one point per block, restrained emphasis, and minimum text size. If any gate fails, refine that page and reassess the same five gates before completion. For a placeholder template deck use pptx_refine_deck; for a visual or branded visual deck use pptx_refine_visual_slide with one complete replacement slide at a time. Never resend, restart, or rebuild the complete deck after a successful generation. The server permits at most three visual refinement rounds. If clipping survives one revision, shorten visible copy or simplify the structure instead of merely tightening spacing or reducing font size.",
            });
            return new CallToolResult
            {
                Content = content,
