...
public static class PowerPointServerInstructions
{
    public static string Build(PptxMcpOptions options)
    {
        var defaultTemplateInstructions = string.IsNullOrWhiteSpace(options.DefaultTemplateId)
            ? """
                No deployment default template is configured. For an ordinary new presentation, complete the staged visual-draft workflow and call pptx_finish_visual_deck with useDefaultTemplate=false. For a new presentation based on an uploaded corporate template, first call pptx_analyze and then pptx_wait_for_job; after completing the same draft workflow, call pptx_finish_branded_visual_deck and pass the uploaded sourceFileId (or latest when the identifier is unavailable).
                """
            : """
                A deployment default template is configured. For ordinary new presentations, complete the staged visual-draft workflow and call pptx_finish_visual_deck with useDefaultTemplate left at its default true value; the MCP server applies the default template without pptx_analyze. Do not ask the user to upload the default template and do not analyze it for ordinary visual generation. If the user explicitly requests strict placement into the default template's existing placeholders, call pptx_analyze with sourceFileId=default and then pptx_wait_for_job to retrieve the startup-cached layout identifiers. If the user explicitly requests no template, set useDefaultTemplate=false when finishing. If the user explicitly requests an attached alternate template, first call pptx_analyze and pptx_wait_for_job for that upload, then complete the draft workflow and call pptx_finish_branded_visual_deck with its sourceFileId (or latest when the identifier is unavailable). An explicit alternate template overrides the deployment default only for that operation.
                """;
