...
- 通常の新規資料は、全体構成、完成ページ数、テーマ、デザイン、テンプレートを決めてから `pptx_start_visual_deck` を1回呼ぶ。既定テンプレートは`templateSourceFileId=default`、テンプレートなしは`none`とし、start成功後は変更しない。返された`draft_id`へ `pptx_add_visual_slides_to_draft` で連続したページを追加する。`visual-v7-author-html`は完成HTML/CSSを持つ2〜4ページ、残り1ページだけは単独で追加し、`startSlideNumber`は省略してサーバーに追番させる。`remaining_slide_count=0`になったらfinishを1回だけ呼ぶ。
...
- 厳密なプレースホルダー資料に問題があれば`pptx_refine_deck`を使う。白紙資料とブランドVisual Deckは`pptx_refine_visual_slide`へ完全な差し替えページを1枚だけ渡し、ジョブ成功後に次の問題ページを`jobId=latest`で直す。修正は逐次累積し、古いジョブからの分岐、複数ページ一括入力、4巡目はサーバーが拒否する。全ページを最大3巡まで自律的に修正し、同じ欠けが残る場合は表示文短縮またはHTML構造単純化を行う。上限後に全体再作成へ戻らず、画像を取得していない場合は視覚確認済みと述べない。
