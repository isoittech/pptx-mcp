...
pptx-mcp ──受付──▶ 永続ジョブキュー（最大3並列、最大10分）
  ...
  │                                ├─ HTML検証: AI設計の静的DOM/CSSをallowlistで再構築
  │                                ├─ dom-to-pptx: 計算済みDOMを編集可能なPPTXへ変換
  │                                ├─ PptxGenJS互換経路: 保存済みv6以前のlineage再生成
...
企業テンプレートはテーマ、マスター、レイアウト、フォント、余白を正本とします。既存スライドは `slide_number + shape_id` で一意指定して更新できます。既存レイアウトへ厳密に内容を流し込む新規生成では、解析で得た `layout_id` とプレースホルダーの `shape_id` を使い、選択された定義済みレイアウトから1〜50枚のスライドを構成します。既定表紙が濃色の場合は導入設定で明示し、1枚目のTitle/Sectionだけを白文字・装飾番号なしの専用DOMオーバーレイにする。添付されたユーザー指定テンプレートへは適用しない。
...
華やかさとブランド保持を両立する場合は、startで企業テンプレートを選び、ドラフト完成後に `pptx_finish_branded_visual_deck` を使います。MCPがaccent色、背景色、文字色、見出し・本文フォントを自動抽出し、startで未指定のテーマ項目だけを補完します。明示色とfontFaceは抽出値より優先されます。`visual-v7-author-html`では会話モデルがページごとの静的HTML/CSSを完成形として設計し、MCPは検証済みDOMを`dom-to-pptx`で編集可能な図形・テキストへ変換して、プレースホルダー0個の白紙レイアウトへ接続します。検収環境では全ページをこの経路に限定し、成功結果のfallback件数を0に固定します。
...
AIには`VisualSlideSpec.authoredHtml`として静的HTML fragmentとCSSを直接設計させますが、実行可能コードは渡させません。MCPはparse5によるtag／attribute allowlistとfail-closedなCSS検査でDOMを再構築し、JavaScript、event属性、外部URL、data URI、任意SVG/XML、ローカルpath、iframe等を拒否します。CSSは全selectorを`.slide`配下に限定し、ページ固有scopeを付けます。Lucideと登録画像はopaque IDからサーバーが解決します。PowerPointのOpen XMLやbrowser optionは公開入力にせず、HTML/CSSを安全な中間表現としてだけ使います。

## 白紙からの視覚的な生成

新規Visual Deckの段階ワークフローでは、Claudeがスライドの意味layoutを計画情報として選びつつ、実際の構図と見た目はページごとのHTML/CSSで設計します。既定テンプレートがない場合、または利用者が明示的に不要とした場合は白紙へ描画します。
...
テーマは `midnight`、`aurora`、`sunset`、`forest`、`minimal`、`ocean`、`berry`、`clay`、`cyber` の9種です。色とフォントは検証済みの範囲で上書きできます。Opusは `design.style`、`density`、`motif`、`variant`、意味layoutを設計条件に使い、各ページの文字、図形、カード、表、余白、配置をHTML/CSSとして完成させます。`visual-v7-author-html`は全ページを`dom-to-pptx` 2.1.1で変換し、固定PptxGenJS rendererへfallbackしません。Lucideは`data-pptx-icon`の承認済み意味ID、登録画像は`data-pptx-asset`のopaque IDからサーバーが安全に埋め込みます。詳細は[ADR 0022](adr/0022-model-authored-html-slides.md)に記録します。
...
意味layoutはモデルのデザイン判断を置き換える金型ではなく、内容構造と検査のための部品語彙です。モデルがストーリー、強調対象、構図、視覚モチーフ、整列、余白をHTML/CSSで決め、サーバーは安全性、編集可能性、ファイル整合性を保証します。6枚以上で構図が4種類未満、同一構図が3枚連続、文字中心ページが過半数の場合などは `design_warnings` を返します。`VisualSlideSpec.speakerNotes`はvisible canvasとは別に発表者ノートへ保存し、refineで省略した場合は元ページから継承します。詳細は [ADR 0010](adr/0010-readable-information-density.md)、[ADR 0018](adr/0018-speaker-notes.md)、[ADR 0022](adr/0022-model-authored-html-slides.md) に記録します。
...
5. 最大3巡で収束させ、視覚確認後にダウンロードリンクを提示する。同じ欠けが残る場合は余白を詰め続けず、表示文の短縮や構造単純化を優先する。
